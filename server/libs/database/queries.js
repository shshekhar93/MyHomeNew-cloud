import { ENTRY_TYPES } from '../constants.js';

const CREATE_ENTRIES_TABLE = `
  CREATE TABLE IF NOT EXISTS ENTRIES
  (
    ID INTEGER PRIMARY KEY AUTOINCREMENT,
    NAME TEXT NOT NULL,
    PATH TEXT NOT NULL,
    TYPE TEXT NOT NULL,
    CATEGORY TEXT,
    CONTENT_TYPE TEXT,
    THUMBNAIL_PATH TEXT
  )
`;
const CREATE_ENTRIES_PATH_NAME_INDEX = `
  CREATE UNIQUE INDEX IF NOT EXISTS ENTRIES_PATH_NAME_INDEX
    ON ENTRIES (
      NAME,
      PATH
    )
`;
const CREATE_ENTRIES_PATH_ONLY_INDEX = `
  CREATE INDEX IF NOT EXISTS ENTRIES_PATH_INDEX
    ON ENTRIES (
      PATH
    )
`;
const CREATE_ENTRIES_CATEGORY_INDEX = `
  CREATE INDEX IF NOT EXISTS ENTRIES_CATEGORY_INDEX
    ON ENTRIES (
      CATEGORY
    )
`;

const CREATE_TAGS_TABLE = `
  CREATE TABLE IF NOT EXISTS TAGS
  (
    TAG TEXT NOT NULL,
    ENTRY_ID NUMBER NOT NULL,
    FOREIGN KEY(ENTRY_ID)
      REFERENCES ENTRIES (ID)
        ON DELETE CASCADE
        ON UPDATE NO ACTION
  )
`;
const CREATE_TAG_NAME_INDEX = `
  CREATE INDEX IF NOT EXISTS TAG_NAME_INDEX
    ON TAGS (
      TAG
    )
`;
const CREATE_TAGS_ENTRY_ID_INDEX = `
  CREATE INDEX IF NOT EXISTS TAGS_ENTRY_ID_INDEX
    ON TAGS (
      ENTRY_ID
    )
`;
const CREATE_TAG_ENTRY_COMBO_INDEX = `
  CREATE UNIQUE INDEX IF NOT EXISTS TAG_NAME_INDEX
    ON TAGS (
      TAG,
      ENTRY_ID
    )
`;

const CREATE_APP_SETTINGS_TABLE = `
  CREATE TABLE IF NOT EXISTS APP_SETTINGS (
    KEY TEXT PRIMARY KEY,
    VALUE TEXT
  )
`;

const CREATE_SETTINGS_KEY_INDEX = `
  CREATE UNIQUE INDEX IF NOT EXISTS SETTINGS_KEY_INDEX
    ON APP_SETTINGS (
      KEY
    )
`;

export const SETUP_QUERIES = [
  CREATE_ENTRIES_TABLE,
  CREATE_ENTRIES_PATH_NAME_INDEX,
  CREATE_ENTRIES_PATH_ONLY_INDEX,
  CREATE_ENTRIES_CATEGORY_INDEX,
  CREATE_TAGS_TABLE,
  CREATE_TAG_NAME_INDEX,
  CREATE_TAGS_ENTRY_ID_INDEX,
  CREATE_TAG_ENTRY_COMBO_INDEX,
  CREATE_APP_SETTINGS_TABLE,
  CREATE_SETTINGS_KEY_INDEX,
];

export const INSERT_DIR_ENTRY_QUERY = `
  INSERT INTO ENTRIES (
    NAME,
    PATH,
    TYPE
  ) VALUES (
    ?,
    ?,
    '${ENTRY_TYPES.DIR}'
  )
`;

export const INSERT_FILE_ENTRY_QUERY = `
  INSERT INTO ENTRIES (
    NAME,
    PATH,
    TYPE,
    CATEGORY,
    CONTENT_TYPE
  ) VALUES (
    ?,
    ?,
    '${ENTRY_TYPES.FILE}',
    ?,
    ?
  )
`;

export const GET_ENTRY_BY_ID = `
  SELECT
    ID,
    NAME,
    PATH,
    TYPE,
    CATEGORY,
    CONTENT_TYPE,
    GROUP_CONCAT(TG.TAG) as TAGS
  FROM ENTRIES EN LEFT OUTER JOIN TAGS TG ON EN.ID=TG.ENTRY_ID
  WHERE ID=?
  GROUP BY 1,2,3,4,5,6
  ORDER BY ID ASC
`;

export const GET_ENTRIES_BY_PATH = `
  SELECT
    ID,
    NAME,
    TYPE,
    CATEGORY,
    CONTENT_TYPE,
    GROUP_CONCAT(TG.TAG) as TAGS
  FROM ENTRIES EN LEFT OUTER JOIN TAGS TG ON EN.ID=TG.ENTRY_ID
  WHERE PATH=?
  GROUP BY 1,2,3,4,5
  ORDER BY ID ASC
`;

// @TODO: Support pagination
export const GET_FILES_BY_CATEGORY = `
  SELECT
    ID,
    NAME,
    TYPE,
    CATEGORY,
    CONTENT_TYPE,
    GROUP_CONCAT(TG.TAG) as TAGS
  FROM ENTRIES EN LEFT OUTER JOIN TAGS TG ON EN.ID=TG.ENTRY_ID
  WHERE CATEGORY=?
    AND TYPE='${ENTRY_TYPES.FILE}'
  GROUP BY 1,2,3,4,5
  ORDER BY ID DESC
`;

// @TODO: Support pagination
export const GET_FILES_BY_TAG = `
  SELECT
    ID,
    NAME,
    TYPE,
    CATEGORY,
    CONTENT_TYPE,
    GROUP_CONCAT(TG.TAG) as TAGS
  FROM ENTRIES EN LEFT OUTER JOIN TAGS TG ON EN.ID=TG.ENTRY_ID
  WHERE ID IN (
    SELECT ENTRY_ID FROM TAGS WHERE TAG=?
  )
  AND TYPE='${ENTRY_TYPES.FILE}'
  GROUP BY 1,2,3,4,5
  ORDER BY ID DESC
`;

export const GET_SETTING_VALUE = `
  SELECT VALUE FROM APP_SETTINGS WHERE KEY=?
`;

export const UPDATE_SETTING_VALUE = `
  INSERT INTO APP_SETTINGS(KEY, VALUE) VALUES (?, ?)
    ON CONFLICT(KEY) DO UPDATE SET VALUE=excluded.VALUE
`;
